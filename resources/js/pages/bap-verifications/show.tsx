    ...
    <>
      ...
      <main className="flex min-w-0 flex-1 flex-col gap-6 p-4 sm:p-6">
        <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
          ...
          {can.start ? (
            <Button asChild>
              <Link href={routes.start(bap.id)} method="post" as="button">
                <Play />
                Mulai {stage.label}
              </Link>
            </Button>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <Card>
            ...
            <CardContent className="grid gap-3 text-sm">
              ...
              <DetailRow label="Dibuat oleh" value={bap.created_by} />
              <DetailRow
                label="Diajukan pada"
                value={
                  bap.submitted_at
          ...
          <Card>
            ...
            <CardContent className="grid gap-3 text-sm">
              <DetailRow
                label="Nomeratur"
                value={formatRange(bap.numerator_start, bap.numerator_end)}
                mono
              />
              <DetailRow
            ...
            <CardContent className="grid gap-3 text-sm">
              {verification ? (
                <>
                  ...
                  <DetailRow
                    label="Dimulai"
                    value={formatDateTime(verification.started_at)}
                  />
                  <DetailRow
                    label="Hasil"
                    value={resultLabel(verification.result, stage.label)}
                  />
                </>
              ) : (
              ...
              <div className="overflow-x-auto">
                <Table className="w-full min-w-155 text-sm">
                  <TableHeader className="text-muted-foreground border-b text-left">
                    <TableRow>
                      <TableHead className="pb-3 font-medium">Box</TableHead>
                      <TableHead className="pb-3 font-medium">
                        Alokasi
                      </TableHead>
                      <TableHead className="pb-3 font-medium">Range</TableHead>
                    ...
                    {bap.segments.map((segment) => (
                      <TableRow key={segment.id}>
                        ...
                        <TableCell className="py-3 text-right tabular-nums">
                          {formatQuantity(segment.quantity)} set
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
          ...
          <Card>
            ...
            <CardContent>
              {bap.cancellations.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  Tidak ada BAP batal/rusak pada pemakaian ini.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  <Table className="w-full min-w-155 text-sm">
        ...
        {stage.is_phase_two ? (
          <section className="grid gap-4">
            ...
            {phaseOneVerification ? (
              ...
              <ValidationNotice>
                Hasil Verifikasi Tahap 1 tidak ditemukan. BAP ini tidak dapat
                diselesaikan pada Tahap 2.
              </ValidationNotice>
            )}
          </section>
        ) : null}
        ...
        {can.complete ? (
          <Card>
            <CardHeader>
              ...
              <p className="text-muted-foreground text-sm">
                Checklist adalah attestasi bahwa pemeriksaan fisik telah
                dilakukan. Nilai sistem tidak dapat diubah dari halaman ini.
              </p>
            </CardHeader>
            <CardContent>
              <form
              ...
              >
                {comparisons.map((comparison, itemIndex) => (
                  <ChecklistRow
                    ...
                    error={
                      form.errors[`checklist.${itemIndex}.actual_quantity`]
                    }
                    rangeStartError={
                      form.errors[
                    ...
                    rangeEndError={
                      form.errors[`checklist.${itemIndex}.actual_numerator_end`]
                    }
                    onChange={(changes) => updateChecklist(itemIndex, changes)}
                  />
                ))}
                ...
                <fieldset className="grid gap-3 rounded-xl border p-4">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Button
                      type="button"
                      variant={
                        form.data.result === "passed" ? "default" : "outline"
                      }
                      onClick={() => form.setData("result", "passed")}
                    >
                      <CheckCircle2 />
                      Lulus verifikasi
                    </Button>
                    <Button
                      ...
                      onClick={() => form.setData("result", "discrepancy")}
                    >
                      <AlertTriangle />
                      Ada selisih
                    </Button>
                  ...
                  {form.data.result === "passed" && mismatches.length > 0 ? (
                    <ValidationNotice>
                      Nilai fisik yang berbeda harus dicatat sebagai selisih;
                      hasil lulus tidak dapat diselesaikan.
                    </ValidationNotice>
                  ) : null}
                  {form.data.result === "discrepancy" &&
                  mismatches.length === 0 ? (
                    <ValidationNotice>
                      Belum ada perbedaan antara nilai sistem dan fisik untuk
                      dicatat.
                    </ValidationNotice>
                  ) : null}
                      ...
                      {mismatches.map((mismatch) => (
                        ...
                        >
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <p className="font-medium">{mismatch.label}</p>
                            <span className="text-destructive text-sm tabular-nums">
                              Selisih {formatDifference(mismatch.difference)}
                            </span>
                          </div>
                          <p className="text-muted-foreground text-sm">
                            Sistem:{" "}
                          ...
                          <div className="grid gap-2">
                            <Label htmlFor={`discrepancy-${mismatch.type}`}>
                              Catatan verifier
                            </Label>
                            <Textarea
                              id={`discrepancy-${mismatch.type}`}
                              value={discrepancyNotes[mismatch.type] ?? ""}
                              onChange={(event) =>
                                setDiscrepancyNotes((current) => ({
                                  ...current,
                                  [mismatch.type]: event.target.value,
                                }))
                              }
                  ...
                  {form.errors.result ? (
                    <ValidationNotice>{form.errors.result}</ValidationNotice>
                  ) : null}
        ...
        <DialogContent>
          ...
          <DialogFooter showCloseButton>
            <Button onClick={submit} disabled={form.processing}>
              {form.processing ? "Menyimpan..." : "Selesaikan verifikasi"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {comparison.type === "numerator" ? (
        <div className="grid gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Input
              ...
              onChange={(event) =>
                onChange({
                  actual_numerator_start: event.target.value.replace(/\D/g, ""),
                })
              }
              inputMode="numeric"
              maxLength={7}
            ...
            <Input
              ...
              onChange={(event) =>
                onChange({
                  actual_numerator_end: event.target.value.replace(/\D/g, ""),
                })
              }
              inputMode="numeric"
              maxLength={7}
        ...
        <div className="grid gap-2">
          ...
          <Input
            ...
            onChange={(event) =>
              onChange({
                actual_quantity: event.target.value.replace(/\D/g, ""),
              })
            }
            inputMode="numeric"
    ...
    >
      <CardHeader>
        <CardTitle className="flex flex-wrap items-center gap-2">
          ...
          {passed ? `${stageLabel} lulus` : `${stageLabel} menemukan selisih`}
        </CardTitle>
      </CardHeader>
              ...
              >
                <div className="flex flex-wrap justify-between gap-2">
                  ...
                  <span className="text-destructive tabular-nums">
                    Selisih {formatDifference(discrepancy.difference)}
                  </span>
                </div>
                ...
                <p className="mt-2 whitespace-pre-wrap">{discrepancy.notes}</p>
              </div>
            ))}
          </div>
        ) : null}
