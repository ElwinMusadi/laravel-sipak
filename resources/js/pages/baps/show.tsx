      ...
      <main className="flex min-w-0 flex-1 flex-col gap-6 p-4 sm:p-6">
        ...
        {bap.status !== "draft" ? (
          <p className="border-primary/25 bg-primary/8 rounded-xl border p-4 text-sm">
            BAP ini bersifat read-only setelah diajukan. Status verifikasi dan
            riwayat pemeriksaannya tersedia pada lifecycle BAP.
          </p>
        ) : (
          <p className="text-muted-foreground rounded-xl border p-4 text-sm">
            Periksa detail pemakaian berikut sebelum mengajukan BAP. Setelah
            diajukan, data tidak dapat diubah bebas.
          </p>
        )}
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <Card>
            ...
            <CardContent className="grid gap-3 text-sm">
              ...
              <DetailRow label="Role aktor" value={bap.creator_role} />
              <DetailRow
                label="Dibuat pada"
        ...
        <Card>
          <CardHeader className="flex-row flex-wrap items-center justify-between gap-3">
            <div className="grid gap-1">
              <CardTitle>Riwayat verifikasi dan klarifikasi</CardTitle>
              <p className="text-muted-foreground text-sm">
                Attempt pemeriksaan dan klarifikasi tersimpan terpisah dari data
                sumber BAP.
              </p>
            </div>
          </CardHeader>
          <CardContent>
            {bap.verification_history.length === 0 ? (
              ...
              <div className="grid gap-3">
                {bap.verification_history.map((verification) => (
                  <div
                    key={verification.id}
                  ...
                  >
                    <div className="grid gap-1 text-sm">
                      <p className="font-medium">
                        {verification.stage_label} · Attempt #
                    ...
                    {verification.clarification ? (
                      verification.clarification.can_view ? (
                        <Button variant="outline" size="sm" asChild>
                          <Link
                            href={showClarification(
                              verification.clarification.id,
                            )}
                          >
                            <MessageSquare data-icon="inline-start" />
                            {verification.clarification.status_label}
                          </Link>
                        </Button>
                      ) : (
        ...
        <Card>
          <CardHeader className="flex-row flex-wrap items-center justify-between gap-3">
            <div className="grid gap-1">
              ...
              <p className="text-muted-foreground text-sm">
                Klasifikasi nomeratur yang tetap dihitung sebagai pemakaian BAP.
              </p>
            </div>
          ...
          <CardContent className="grid gap-4">
            ...
            {bap.cancellations.items.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                Belum ada nomeratur batal atau rusak pada BAP ini.
              </p>
            ) : (
              ...
              <Table className="w-full min-w-152 text-sm">
                <TableHeader className="text-muted-foreground border-b text-left">
                  <TableRow>
                    ...
                    <TableHead className="px-2 py-3 font-medium">Box</TableHead>
                    <TableHead className="px-2 py-3 font-medium">
                      Nomeratur
                    </TableHead>
